...
export default function ShareButtons({ url, title, className = '' }: ShareButtonsProps) {
  ...
  return (
    <div className={`flex items-center gap-2 ${className}`}>
      <Share2 size={16} className="text-gray-400 mr-1" />
      ...
      {options.map((option) => (
        <button
          ...
          className={`p-2 rounded-full text-gray-500 transition-colors cursor-pointer ${option.color}`}
        >
          {option.icon}
        </button>
      ))}
      ...
      >
        {copied ? <Check size={18} className="text-[#2E7D32]" /> : <Link size={18} />}
      </button>

      {copied && (
        <span className="text-xs text-[#2E7D32] font-medium">Copied!</span>
      )}
    </div>
  );
}
